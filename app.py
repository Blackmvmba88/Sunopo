...
def initialize_session_store():
    """Build the Redis-backed session store with explicit failure categories."""
    if not REDIS_URL:
        logger.warning("Session store disabled: REDIS_URL is not configured")
        return None
    ...
    try:
        store = SessionStore(
            ...
            fernet_key=SESSION_FERNET_KEY,
        )
    except (TypeError, ValueError) as exc:
        logger.error("Session store configuration error: %s", type(exc).__name__)
        return None
    ...
    try:
        ...
        logger.warning("Session store Redis connectivity error: %s", type(exc).__name__)
        return None
